...
const Navbar = () => {
  ...
  const bgColorsBody = ["#ffb457", "#ff96bd", "#9999fb", "#f5d467", "#7df0ce"];
  const menuItems = [
    {
      icon: (
  ...
  const offsetMenuBorder = () => {
    if (
      menuRef.current &&
      menuBorderRef.current &&
      menuRef.current.children[activeItem]
    ) {
      const activeElement = menuRef.current.children[activeItem];
  ...
  return (
    <div>
      <h1 className="text-6xl neon">Mithu Chowdhury</h1>
      <nav className="menu" ref={menuRef}>
        {menuItems.map((item, index) => (
          <button
            key={index}
            className={`menu__item ${index === activeItem ? "active" : ""}`}
            onClick={() => clickItem(index, item.text)}
